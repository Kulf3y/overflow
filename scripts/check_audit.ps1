...
Write-Host "Running audit tests..." -ForegroundColor Cyan

$env:PYTHONPATH = "$pythonSrc"
python -m unittest discover -s "$testsPath" -p "test_audit.py"

if ($LASTEXITCODE -ne 0) {
    Write-Host "Audit tests failed." -ForegroundColor Red
    exit 1
}

Write-Host "Audit tests passed." -ForegroundColor Green
